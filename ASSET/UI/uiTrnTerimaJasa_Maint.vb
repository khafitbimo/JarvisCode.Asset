...
#Region " Additional Overrides "
    ...
    Private Sub uiTrnTerimaJasa_Maintenance_AppUser(ByVal status As String)
        ...
        ' ARI MDP2 20160616
        '=====================================================================================================================================================================
        If Me._USERTYPE <> "acc" Then ' _MODULE_TYPE = ModuleType.PURCHASE And Me._USERTYPE <> "acc" Then
            ...
            Dim vw As DataView = New DataView(Me.tbl_TrnTerimajasadetil)
            Dim check() As DataRow = vw.ToTable(True, "terimajasa_id", "order_id", "orderdetil_line").Select(" order_id <> ''")
            ...
            If check.Length > 0 Then
                ...
                For Each dtRow As DataRow In objTblTemp1.Rows
                    AmountNEW = 0
                    objTblTemp2.Clear()
                    check = Me.tbl_TrnTerimajasadetil.Select(String.Format(" order_id = '{0}' AND orderdetil_line = {1}", dtRow("order_id").ToString, dtRow("orderdetil_line")))

                    If check.Length > 0 Then
                        objTblTemp2 = check.CopyToDataTable

                        For Each dtRow2 As DataRow In objTblTemp2.Rows
                            AmountNEW = AmountNEW + ((dtRow2("terimajasadetil_foreign") - dtRow2("terimajasadetil_disc")) * dtRow2("terimajasadetil_qty"))
                        Next

                        AmountNEW = Math.Floor(AmountNEW * 100) / 100

                        clsTerimaBarang.CheckingBPJAmountWithDiscPerLine(objTblChecking, dtRow("terimajasa_id"), dtRow("order_id"), dtRow("orderdetil_line"), AmountNEW)

                        If objTblChecking.Rows.Count > 0 Then
                            If objTblChecking.Rows(0).Item("terimajasa_totalsisa") < 0 Then
                                MsgBox("Amount melebihi Order !!")
                                Exit Sub
                            End If
                        End If
                    End If
    ...
    Private Sub uiTrnTerimaJasa_Maintenance_AppBma(ByVal status As String)
        ...
        Try
            ...
            'oCmJurnal.Parameters.Add("@channel_id", System.Data.OleDb.OleDbType.VarWChar, 20).Value = Me.DgvTrnTerimajasa.Item("channel_id", DgvTrnTerimajasa.CurrentRow.Index).Value
...
#Region " User Defined Function "
    ...
    Private Function uiTrnTerimaJasa_Maintenance_OpenRow(ByVal rowIndex As Integer) As Boolean
        ...
        Try
            ...
            clsApplicationRole.SetAppRole(dbConn, cookie)
            'clsApplicationRole.SetAppRole(dbConnAsset, cookie1)
            Me.uiTrnTerimaJasa_Maintenance_OpenRowMaster(channel_id, terimajasa_id, dbConn)
            Me.uiTrnTerimaJasa_Maintenance_OpenRowDetil(channel_id, terimajasa_id, dbConn)
            Me.uiTrnTerimaJasa_Maintenance_OpenRowDetilUsed(channel_id, terimajasa_id, dbConn)
    ...
    Private Function uiTrnTerimaJasa_Maintenance_OpenRowMaster(ByVal channel_id As String, ByVal terimajasa_id As String, ByVal dbConn As OleDb.OleDbConnection) As Boolean
        ...
        Try
            ...
            If Me.tbl_TrnTerimajasa_Temp.Rows.Count > 0 Then
                ...
                'dbConnStart.Open()
                'clsApplicationRole.SetAppRole(dbConnStart, cookie2)
                Me.uiTrnMaintenanceOrder2_OpenRowDetil(channel_id, tbl_TrnTerimajasa_Temp.Rows(0).Item("order_id"), dbConn)
                'clsApplicationRole.UnsetAppRole(dbConnStart, cookie2)
                'dbConnStart.Close()
...
#End Region
    ...
    Private Sub btn_order_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_order.Click
        ...
        Try
            ...
            If Me.DgvTrnTerimajasadetil.Rows.Count = 0 Then
                Dim dlg As dlgTrnTerimajasa_Select_Maintenance = New dlgTrnTerimajasa_Select_Maintenance(Me.DSNFrm, Me.obj_Rekanan_id.SelectedValue, Me.tbl_MstRekanan.Copy, Me._STRUKTUR_UNIT)
    ...
    Private Sub Btn_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_Add.Click
        ...
        If retObj IsNot Nothing Then
            ...
            For row_index = 0 To tblDetil.Rows.Count - 1
                ...
                row.Item("terimajasadetil_qty_usage") = tblDetil.Rows(row_index).Item("mo_outstanding") 'tblDetil.Rows(row_index).Item("orderdetil_qty")
                ...
                Dim orderdetil_foreign As Decimal
                Dim mo_outstanding As Decimal
                Dim orderdetil_discount As Decimal
                Dim orderdetil_pphpercent As Decimal
                Dim orderdetil_ppnpercent As Decimal
                Dim terimajasadetil_pphforeign As Decimal
                Dim terimajasadetil_ppnforeign As Decimal
                Dim orderdetil_foreignrate As Decimal
                Dim orderdetil_qty As Decimal
                Dim terimajasadetil_pphidrreal As Decimal
                Dim terimajasadetil_ppnidrreal As Decimal


                orderdetil_foreign = clsUtil.IsDbNull(tblDetil.Rows(row_index).Item("orderdetil_foreign"), 0)
                mo_outstanding = clsUtil.IsDbNull(tblDetil.Rows(row_index).Item("mo_outstanding"), 0)
                orderdetil_discount = clsUtil.IsDbNull(tblDetil.Rows(row_index).Item("orderdetil_discount"), 0)
                orderdetil_pphpercent = clsUtil.IsDbNull(tblDetil.Rows(row_index).Item("orderdetil_pphpercent"), 0)
                orderdetil_ppnpercent = clsUtil.IsDbNull(tblDetil.Rows(row_index).Item("orderdetil_ppnpercent"), 0)
                terimajasadetil_pphforeign = clsUtil.IsDbNull(row.Item("terimajasadetil_pphforeign"), 0)
                terimajasadetil_ppnforeign = clsUtil.IsDbNull(row.Item("terimajasadetil_ppnforeign"), 0)
                orderdetil_foreignrate = clsUtil.IsDbNull(tblDetil.Rows(row_index).Item("orderdetil_foreignrate"), 0)
                orderdetil_qty = clsUtil.IsDbNull(tblDetil.Rows(row_index).Item("orderdetil_qty"), 0)

                If tblDetil.Rows(row_index).Item("currency_id") = 1 Then

                    row.Item("terimajasadetil_pphforeign") = Math.Round(( _
                                                                            (orderdetil_foreign - orderdetil_discount) * mo_outstanding) _
                                                                                * (orderdetil_pphpercent / 100), _
                                                                                    0, MidpointRounding.AwayFromZero)
                    row.Item("terimajasadetil_ppnforeign") = Math.Round(( _
                                                                            (orderdetil_foreign - orderdetil_discount) * mo_outstanding) _
                                                                                * (orderdetil_ppnpercent / 100), _
                                                                                    0, MidpointRounding.AwayFromZero)

                    row.Item("terimajasadetil_totalforeign") = Math.Round(( _
                                                                            (orderdetil_foreign - orderdetil_discount) * mo_outstanding) _
                                                                                - (terimajasadetil_pphforeign) + (terimajasadetil_ppnforeign) _
                                                                                    , 0, MidpointRounding.AwayFromZero)
                Else
                    row.Item("terimajasadetil_pphforeign") = Math.Round(( _
                                                                            (orderdetil_foreign - orderdetil_discount) * mo_outstanding) _
                ...
                row.Item("terimajasadetil_pphidrreal") = Math.Round(( _
                                                                        (orderdetil_foreign - orderdetil_discount) * mo_outstanding) _
                                                                            * (orderdetil_pphpercent / 100) * (orderdetil_foreignrate) _
                                                                                , 0, MidpointRounding.AwayFromZero)
                row.Item("terimajasadetil_ppnidrreal") = Math.Round(( _
                                                                        (orderdetil_foreign - orderdetil_discount) * mo_outstanding) _
                                                                            * (orderdetil_ppnpercent / 100) * (orderdetil_foreignrate) _
                                                                                , 0, MidpointRounding.AwayFromZero)

                terimajasadetil_pphidrreal = row.Item("terimajasadetil_pphidrreal")
                terimajasadetil_ppnidrreal = row.Item("terimajasadetil_ppnidrreal")

                row.Item("terimajasadetil_totalidrreal") = Math.Round( _
                                                                        (orderdetil_foreign - orderdetil_discount) * mo_outstanding * orderdetil_foreignrate - _
                                                                            terimajasadetil_pphidrreal + terimajasadetil_ppnidrreal _
                                                                                    , 0, MidpointRounding.AwayFromZero)
    ...
    Private Sub DgvTrnTerimajasadetil_CellValidated(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgvTrnTerimajasadetil.CellValidated
        ...
        Dim terimajasadetil_foreign As Decimal
        Dim terimajasadetil_disc As Decimal
        Dim terimajasadetil_qty_usage As Decimal
        Dim terimajasadetil_pphpersen As Decimal
        Dim terimajasadetil_foreignrate As Decimal
        Dim terimajasadetil_ppnpersen As Decimal

        terimajasadetil_foreign = clsUtil.IsDbNull(Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index).Cells("terimajasadetil_foreign").Value, 0)
        terimajasadetil_disc = clsUtil.IsDbNull(Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index).Cells("terimajasadetil_disc").Value, 0)
        terimajasadetil_qty_usage = clsUtil.IsDbNull(Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index).Cells("terimajasadetil_qty_usage").Value, 0)
        terimajasadetil_pphpersen = clsUtil.IsDbNull(Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index).Cells("terimajasadetil_pphpersen").Value, 0)
        terimajasadetil_foreignrate = clsUtil.IsDbNull(DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index).Cells("terimajasadetil_foreignrate").Value, 0)
        terimajasadetil_ppnpersen = clsUtil.IsDbNull(Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index).Cells("terimajasadetil_ppnpersen").Value, 0)

        pphAmount = ((terimajasadetil_foreign - terimajasadetil_disc) * terimajasadetil_qty_usage) _
                                    * (terimajasadetil_pphpersen / 100)

        pphAmountIDR = ((terimajasadetil_foreign - terimajasadetil_disc) * terimajasadetil_qty_usage) _
                    * (terimajasadetil_pphpersen / 100) * (terimajasadetil_foreignrate)

        ppnAmount = ((terimajasadetil_foreign - terimajasadetil_disc) * terimajasadetil_qty_usage) _
                                       * (terimajasadetil_ppnpersen / 100)

        ppnAmountIDR = ((terimajasadetil_foreign - terimajasadetil_disc) * terimajasadetil_qty_usage) _
                    * (terimajasadetil_ppnpersen / 100) * (terimajasadetil_foreignrate)
        ...
        totalAmountIDR = (terimajasadetil_foreign - terimajasadetil_disc) * terimajasadetil_foreignrate * _
                          terimajasadetil_qty_usage - clsUtil.IsDbNull(pphAmountIDR, 0) + clsUtil.IsDbNull(ppnAmountIDR, 0)

        If Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index).Cells("currency_id").Value = 1 Then
            With Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index)
                .Cells("terimajasadetil_pphforeign").Value = Math.Round(pphAmount, 0, MidpointRounding.AwayFromZero)
                .Cells("terimajasadetil_ppnforeign").Value = Math.Round(ppnAmount, 0, MidpointRounding.AwayFromZero)
                .Cells("terimajasadetil_totalforeign").Value = Math.Round(totalAmount, 0, MidpointRounding.AwayFromZero)
            End With
        Else
            With Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index)
                .Cells("terimajasadetil_pphforeign").Value = Math.Round(pphAmount, 2, MidpointRounding.AwayFromZero)
                .Cells("terimajasadetil_ppnforeign").Value = Math.Round(ppnAmount, 2, MidpointRounding.AwayFromZero)
                .Cells("terimajasadetil_totalforeign").Value = Math.Round(totalAmount, 2, MidpointRounding.AwayFromZero)
            End With
        End If

        With Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index)
            .Cells("terimajasadetil_pphidrreal").Value = Math.Round(pphAmountIDR, 0, MidpointRounding.AwayFromZero)
            .Cells("terimajasadetil_ppnidrreal").Value = Math.Round(ppnAmountIDR, 0, MidpointRounding.AwayFromZero)
            .Cells("terimajasadetil_totalidrreal").Value = Math.Round(totalAmountIDR, 0, MidpointRounding.AwayFromZero)
        End With
    End Sub
    ...
    Private Sub DgvTrnTerimajasadetil_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DgvTrnTerimajasadetil.CellValueChanged
        Select Case e.ColumnIndex
            Case Me.DgvTrnTerimajasadetil.Columns("terimajasadetil_qty").Index
                ...
                If Me.DgvTrnTerimajasadetil.Rows(Me.DgvTrnTerimajasadetil.CurrentRow.Index).Cells("currency_id").Value = 1 Then
                    With Me.DgvTrnTerimajasadetil.Rows(e.RowIndex)
                        .Cells("terimajasadetil_pphforeign").Value = Math.Round(pphAmount, 0, MidpointRounding.AwayFromZero)
                        .Cells("terimajasadetil_ppnforeign").Value = Math.Round(ppnAmount, 0, MidpointRounding.AwayFromZero)
                        .Cells("terimajasadetil_totalforeign").Value = Math.Round(totalAmount, 0, MidpointRounding.AwayFromZero)
                    End With
                ...
                With Me.DgvTrnTerimajasadetil.Rows(e.RowIndex)
                    ...
                    .Cells("terimajasadetil_ppnidrreal").Value = Math.Round(ppnAmountIDR, 0, MidpointRounding.AwayFromZero)
                    .Cells("terimajasadetil_totalidrreal").Value = Math.Round(totalAmountIDR, 0, MidpointRounding.AwayFromZero)
                    .Cells("terimajasadetil_qty_usage").Value = qty_usage
                End With
        End Select
    End Sub
...
#Region "PRINT"
    Private Function GenerateDataHeader() As ArrayList
        ...
        objPrintHeader = New DataSource.clsRptTerimaJasaMaintenance(Me.DSNFrm)
        Dim odatafiller As clsDataFiller = New clsDataFiller(Me.DSNFrm)
        ...
        With objPrintHeader
            ...
            'odatafiller.DataFillAsset(Me.DSNAsset, tbl_PrintDetil, "as_TrnTerimajasadetil_Select", " terimajasa_id = '" & .terimajasa_id & "'")
            GenerateDataDetail()
        End With
    ...
    Private Sub GenerateDataDetail()
        ...
        For i = 0 To Me.tbl_PrintDetil.Rows.Count - 1
            objDetil = New DataSource.clsRptTerimaJasaMaintenanceDetil(Me.DSNFrm)
            With objDetil
    ...
    Private Function uiTrnTerimaJasa_Maintenance_Print() As Boolean
        ...
        If Me.DgvTrnTerimajasa.Rows(Me.DgvTrnTerimajasa.CurrentRow.Index).Cells("terimajasa_appspv").Value = 1 Then
            ...
            If ket = "Internal" Then
                objReportH.ReportEmbeddedResource = "ASSET.RptTerimaJasaMaintenance2.rdlc"
            Else
    ...
    Private Function uiTrnTerimaJasa_Maintenance_PrintPreview() As Boolean
        ...
        If Me.DgvTrnTerimajasa.Rows(Me.DgvTrnTerimajasa.CurrentRow.Index).Cells("terimajasa_appspv").Value = 1 Then
            ...
            Dim frmPrint As dlgRptTerimaJasaMaintenance = New dlgRptTerimaJasaMaintenance(Me.DSNFrm, Me.SptServer, Me._CHANNEL, ket)
            Dim criteria As String = String.Empty
